// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.24.0
// source: item.sql

package sqlc

import (
	"context"
)

const allItemsFromList = `-- name: AllItemsFromList :many
SELECT id, list_id, content, checked, sort FROM items
WHERE list_id = ?
ORDER BY sort ASC
`

func (q *Queries) AllItemsFromList(ctx context.Context, listID int64) ([]Item, error) {
	rows, err := q.db.QueryContext(ctx, allItemsFromList, listID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Item
	for rows.Next() {
		var i Item
		if err := rows.Scan(
			&i.ID,
			&i.ListID,
			&i.Content,
			&i.Checked,
			&i.Sort,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const allItemsFromListPaginated = `-- name: AllItemsFromListPaginated :many
SELECT id, list_id, content, checked, sort FROM items
WHERE list_id = ?
ORDER BY sort ASC
LIMIT ?
OFFSET ?
`

type AllItemsFromListPaginatedParams struct {
	ListID int64
	Limit  int64
	Offset int64
}

func (q *Queries) AllItemsFromListPaginated(ctx context.Context, arg AllItemsFromListPaginatedParams) ([]Item, error) {
	rows, err := q.db.QueryContext(ctx, allItemsFromListPaginated, arg.ListID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Item
	for rows.Next() {
		var i Item
		if err := rows.Scan(
			&i.ID,
			&i.ListID,
			&i.Content,
			&i.Checked,
			&i.Sort,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const createListItem = `-- name: CreateListItem :one
INSERT INTO items (list_id, content, sort)
VALUES (?, ?, ?)
RETURNING id, list_id, content, checked, sort
`

type CreateListItemParams struct {
	ListID  int64
	Content string
	Sort    int64
}

func (q *Queries) CreateListItem(ctx context.Context, arg CreateListItemParams) (Item, error) {
	row := q.db.QueryRowContext(ctx, createListItem, arg.ListID, arg.Content, arg.Sort)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.ListID,
		&i.Content,
		&i.Checked,
		&i.Sort,
	)
	return i, err
}

const deleteListItem = `-- name: DeleteListItem :exec
DELETE FROM items
WHERE id = ?
`

func (q *Queries) DeleteListItem(ctx context.Context, id int64) error {
	_, err := q.db.ExecContext(ctx, deleteListItem, id)
	return err
}

const getListItem = `-- name: GetListItem :one
SELECT id, list_id, content, checked, sort FROM items
WHERE id = ? LIMIT 1
`

func (q *Queries) GetListItem(ctx context.Context, id int64) (Item, error) {
	row := q.db.QueryRowContext(ctx, getListItem, id)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.ListID,
		&i.Content,
		&i.Checked,
		&i.Sort,
	)
	return i, err
}

const updateListItemChecked = `-- name: UpdateListItemChecked :one
UPDATE items
SET checked =?
WHERE id =?
RETURNING id, list_id, content, checked, sort
`

type UpdateListItemCheckedParams struct {
	Checked bool
	ID      int64
}

func (q *Queries) UpdateListItemChecked(ctx context.Context, arg UpdateListItemCheckedParams) (Item, error) {
	row := q.db.QueryRowContext(ctx, updateListItemChecked, arg.Checked, arg.ID)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.ListID,
		&i.Content,
		&i.Checked,
		&i.Sort,
	)
	return i, err
}

const updateListItemContent = `-- name: UpdateListItemContent :one
UPDATE items
SET content = ?
WHERE id = ?
RETURNING id, list_id, content, checked, sort
`

type UpdateListItemContentParams struct {
	Content string
	ID      int64
}

func (q *Queries) UpdateListItemContent(ctx context.Context, arg UpdateListItemContentParams) (Item, error) {
	row := q.db.QueryRowContext(ctx, updateListItemContent, arg.Content, arg.ID)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.ListID,
		&i.Content,
		&i.Checked,
		&i.Sort,
	)
	return i, err
}

const updateListItemSort = `-- name: UpdateListItemSort :one
UPDATE items
SET sort = ?
WHERE id = ?
RETURNING id, list_id, content, checked, sort
`

type UpdateListItemSortParams struct {
	Sort int64
	ID   int64
}

func (q *Queries) UpdateListItemSort(ctx context.Context, arg UpdateListItemSortParams) (Item, error) {
	row := q.db.QueryRowContext(ctx, updateListItemSort, arg.Sort, arg.ID)
	var i Item
	err := row.Scan(
		&i.ID,
		&i.ListID,
		&i.Content,
		&i.Checked,
		&i.Sort,
	)
	return i, err
}
